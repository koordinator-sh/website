...
status: {}
```
2. Create a Pod to request the use of Ascend cards. Examples below demonstrate both full-card and virtual-card scenarios.
  ...
  - command:
    ...
    resources:
      ...
      requests:
        ...
        huawei.com/npu-cpu: "3"
```
3. Enter the container (`kubectl exec -it {pod-name} -- bash`), and run `npu-smi` inside to check card usage. If the data is displayed correctly, the card has been successfully allocated to the Pod.
