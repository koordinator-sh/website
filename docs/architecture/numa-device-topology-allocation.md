...
Unlike `NUMATopologyPolicy` — an admission constraint whose pod/node mismatch is rejected with `ErrNotMatchNUMATopology` — the allocate strategy is a **placement preference**: a pod annotation *overrides* the node label instead of conflicting with it. At the pod level **only `DistributeEvenly` is honored** (`MostAllocated` / `LeastAllocated` remain node-level strategies), and PreFilter rejects any other pod-level value with a diagnosable status. The even spread is **gated** — it applies only when the node actually exposes NUMA topology and the pod is not confined to a single NUMA node, so a topology-less node keeps its prior `None` behavior and `Restricted` / `SingleNUMANode` admission is never changed by the spread.
