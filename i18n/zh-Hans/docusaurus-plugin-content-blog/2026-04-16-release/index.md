...
- **集群级预占模式**：通过 `enableClusterMode`，将预占能力从“绑定到特定匹配节点”升级为“基于整个集群的可用容量进行预占”，预占的 Pod 在绑定时可在集群中任意具备可用容量的节点上运行，而不再局限于 Reservation 最初匹配的某几台节点。
- **单个预留中支持多个预占 Pod**：一个 Reservation 可以覆盖整个 GangGroup 或副本集。
- **预占优先级**：通过 `pod.koordinator.sh/pre-allocatable-priority` 注解精细控制候选 Pod 是否可被预占。
...
- **Descheduler 改进**：支持跳过 Eviction Gates、支持自定义 Priority 插件、修复 AnomalyCondition 检查 Bug、NodePool 继承顶层默认配置、`LowNodeLoad` 基于 Raw Allocatable 计算阈值。
