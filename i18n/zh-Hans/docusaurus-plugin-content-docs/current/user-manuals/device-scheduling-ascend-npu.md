...
spec:
  ...
  - health: true
    ...
    resources:
      ...
      koordinator.sh/gpu-core: "100" # 百分比
...
status: {}
```
2. 创建 Pod 申请使用昇腾卡，样例如下，示例中申请了整卡和虚拟卡场景。
- 注意
...
```
3. 进入容器内部( `kubectl exec -it {pod-name} --bash`)，在容器内部执行 `npu-smi` 命令查看卡的使用情况。如果能够正常显示数据，表示卡已经成功分配到 Pod 中。
