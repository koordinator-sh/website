...
```yaml
controller:
  # 队列分组插件：elasticquotav2、elasticquota 或 resourceQuota
  queueGroupPlugin: elasticquotav2
  # 启用严格出队模式（出队前需要调度器确认）
  enableResourceCheckWithScheduler: false
  # 启用严格优先级（高优先级队列始终优先调度）
...
### 使用 ResourceQuota 快速开始

此示例演示使用 Kubernetes 原生 ResourceQuota 的多队列作业调度。

#### 1. 创建队列和资源配额

所有 `Queue` 资源必须创建在 `koord-queue` 命名空间（控制器所在的命名空间）下。`ResourceQuota` 和作业在各自的命名空间中创建。
...
创建带有 suspend 注解的 TFJob（Koord-Queue 的 Job Extension 会自动处理）。对于 TFJob/PyTorchJob，使用 `scheduling.x-k8s.io/suspend` 注解；对于 Kubernetes Job，使用 `spec.suspend: true`：
...
| 字段 | 类型 | 描述 |
|------|------|------|
| `queuePolicy` | `string` | 排队策略：`FIFO` 或 `Priority`。 |
