...
3. 从 Koordinator v1.7 开始，koord-manager 的 `EnableSyncGPUSharedResource` 默认设置为 `false`。在 GPU 共享场景下，需要将该值设置为 `true`。可以通过在 koord-manager 的 args 参数中添加 `-feature-gates=EnableSyncGPUSharedResource=true` 来启用。
