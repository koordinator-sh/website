...
它在**已经满足请求**的 NUMA 节点之间做选择，并按明确的优先级解析：

- **Pod annotation** `koordinator.sh/numa-topology-spec` → `NUMATopologySpec.numaAllocateStrategy`
...
| `DistributeEvenly` | 将 Pod 的 cpu/memory 打散到**尽可能多的 NUMA 节点**（优先最宽的 NUMA affinity），而非塞进最少的几个。 | 类似 interleave 的放置，适合内存在运行时跨 NUMA 节点交织（如 `mbind` `MPOL_INTERLEAVE`）的负载，保持每 NUMA 容量均衡。 |

与 `NUMATopologyPolicy` 不同——后者是准入约束，pod/node 不一致会以 `ErrNotMatchNUMATopology` 拒绝——allocate strategy 是一种**放置偏好**：Pod annotation *覆盖* Node label，而非与之冲突。在 Pod 级**仅 `DistributeEvenly` 生效**（`MostAllocated` / `LeastAllocated` 仍是节点级策略），其他 Pod 级取值会在 PreFilter 被拒绝并给出可诊断的 status。该均匀打散是**有门控的**——仅当节点确实暴露 NUMA 拓扑、且 Pod 未被限制在单个 NUMA 节点时才生效，因此没有拓扑的节点保持原有的 `None` 行为，`Restricted` / `SingleNUMANode` 的准入结果也不会因打散而改变。
